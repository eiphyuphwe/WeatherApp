    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
       ...
       // holder.itemView.setOnClickListener(clickListener)
        ...
        holder.itemView?.labelWind.text = "Wind"
            ...
            holder.itemView?.valueWind?.text = values?.get(0)?.wind?.speed?.let { Convertion.roundOffDecimal(Convertion.convertWindMPH(it))?.toString() + " mph" }

        holder.itemView?.txtDate?.text = values?.get(0)?.dt?.let { Convertion.getDay(it)?.name }
        ...
        holder.itemView?.textViewTemp?.text =
            values?.get(0)?.main?.temp?.let { Convertion.convertKelvinTocelsius(it)?.toString() +"°"}
